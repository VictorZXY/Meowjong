...
def validate_dataset(dataset_path):
    path, dirs, files = next(os.walk(os.path.join(dataset_path, 'kan_2019')))
...
if __name__ == '__main__':
    ...
    with tf.device('/physical_device:CPU:0'):
        ...
        image_files = tf.data.Dataset.list_files(os.path.join(
            dataset_path, 'kan_2019/*.png'))
        labels = load_data('kan_actions_2019.csv')
        for i in range(len(labels)):
            assert labels['image'][i] == 'kan_2019_' + str(i + 1) + '.png'
        print('Dataset OK')
        print()
